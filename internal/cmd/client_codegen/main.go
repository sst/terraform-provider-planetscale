...
func realMain(specFilepath string) error {
	...
	f.PackageComment("Code generated by `github.com/sst/terraform-provider-planetscale/internal/cmd/client_codegen` DO NOT EDIT")

	genClientStruct(f, spec)
	genErrorStruct(f)

	keys := maps.Keys(spec.Definitions)
	sort.Strings(keys)
	...
	for _, code := range resCodes {
		...
		if code < 400 {
			...
			if respSchema != nil && respSchema.Ref.GetURL() != nil && respSchema.Ref.GetURL().Fragment != "" {
				...
				genEmbedStruct(f, resBodyStructName, snakeToCamel(defnName))
